...
public class CreateDLRCEFXStudyCommand {
	...
	/**
	 * Creates a CompoundCommand for adding a new system structure based on the
	 * given Concept.
	 *
	 * <p>
	 * This method generates a system structure consisting of a ConfigurationTree, a
	 * SubSystem, and an Equipment. It then creates commands to add these structural
	 * elements as child elements in a hierarchical manner. Finally, it records
	 * commands to add system, sub-system, and equipment parameters using the
	 * DLRCEFXStudyCommandHelper.
	 * </p>
	 *
	 * @param parent  The parent EObject to which the system structure will be
	 *                added.
	 * @param concept The Concept used to create the system structure.
	 * @param domain  The VirSatTransactionalEditingDomain in which the command will
	 *                be executed.
	 * @return A CompoundCommand representing the operation to create the system
	 *         structure.
	 */
	public static CompoundCommand create(EObject parent, Concept conceptPs, Concept conceptCefx,
		...
		ElementConfiguration powerEquipment = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementConfiguration(conceptPs);
		ElementConfiguration structureEquipment = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementConfiguration(conceptPs);
		ElementConfiguration aocsEquipment = DLRCEFXStudyCommandHelper.createEquipmentAsElementConfiguration(conceptPs);
		ElementConfiguration payloadEquipment = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementConfiguration(conceptPs);
		ElementConfiguration dataHandlingEquipment = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementConfiguration(conceptPs);
		ElementDefinition powerElementDefinition = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementDefinition(conceptPs);
		ElementDefinition structureElementDefinition = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementDefinition(conceptPs);
		ElementDefinition aocsElementDefinition = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementDefinition(conceptPs);
		ElementDefinition payloadElementDefinition = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementDefinition(conceptPs);
		ElementDefinition dataHandlingElementDefinition = DLRCEFXStudyCommandHelper
				.createEquipmentAsElementDefinition(conceptPs);
		// Create default subsystems and assign disciplines
		ProductTreeDomain powerProductTreeDomain = DLRCEFXStudyCommandHelper
				.createSubSystemAsProductTreeDomain(conceptPs);
		powerProductTreeDomain.setName(POWER_NAME);
		ProductTreeDomain structureProductTreeDomain = DLRCEFXStudyCommandHelper
				.createSubSystemAsProductTreeDomain(conceptPs);
		structureProductTreeDomain.setName(STRUCTURE_NAME);
		ProductTreeDomain aocsProductTreeDomain = DLRCEFXStudyCommandHelper
				.createSubSystemAsProductTreeDomain(conceptPs);
		aocsProductTreeDomain.setName(AOCS_NAME);
		ProductTreeDomain payloadProductTreeDomain = DLRCEFXStudyCommandHelper
				.createSubSystemAsProductTreeDomain(conceptPs);
		payloadProductTreeDomain.setName(PAYLOAD_NAME);
		ProductTreeDomain dataHandlingProductTreeDomain = DLRCEFXStudyCommandHelper
				.createSubSystemAsProductTreeDomain(conceptPs);
		dataHandlingProductTreeDomain.setName(DATAHANDLING_NAME);
		ElementConfiguration powerSubsystem = DLRCEFXStudyCommandHelper
				.createSubSystemAsElementConfiguration(conceptPs);
		powerSubsystem.setName(POWER_NAME);
		ElementConfiguration structureSubsystem = DLRCEFXStudyCommandHelper
				.createSubSystemAsElementConfiguration(conceptPs);
		...
		ElementConfiguration payloadSubsystem = DLRCEFXStudyCommandHelper
				.createSubSystemAsElementConfiguration(conceptPs);
		payloadSubsystem.setName(PAYLOAD_NAME);
		ElementConfiguration dataHandlingSubsystem = DLRCEFXStudyCommandHelper
				.createSubSystemAsElementConfiguration(conceptPs);
		dataHandlingSubsystem.setName(DATAHANDLING_NAME);

		// Create a CompoundCommand to store sub-commands
		CompoundCommand cmd = new CompoundCommand();
		// Get references for the disciplines
		...
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(parent,
				productTree.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(parent, system.getStructuralElementInstance(),
				domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(productTree.getStructuralElementInstance(),
				powerProductTreeDomain.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(
				powerProductTreeDomain.getStructuralElementInstance(),
				powerElementDefinition.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(productTree.getStructuralElementInstance(),
				aocsProductTreeDomain.getStructuralElementInstance(), domain));
		cmd.append(
				DLRCEFXStudyCommandHelper.createAddChildSEICommand(aocsProductTreeDomain.getStructuralElementInstance(),
						aocsElementDefinition.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(productTree.getStructuralElementInstance(),
				payloadProductTreeDomain.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(
				payloadProductTreeDomain.getStructuralElementInstance(),
				payloadElementDefinition.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(productTree.getStructuralElementInstance(),
				structureProductTreeDomain.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(
				structureProductTreeDomain.getStructuralElementInstance(),
				structureElementDefinition.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(productTree.getStructuralElementInstance(),
				dataHandlingProductTreeDomain.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(
				dataHandlingProductTreeDomain.getStructuralElementInstance(),
				dataHandlingElementDefinition.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(system.getStructuralElementInstance(),
				powerSubsystem.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(system.getStructuralElementInstance(),
				aocsSubsystem.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(system.getStructuralElementInstance(),
				payloadSubsystem.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(system.getStructuralElementInstance(),
				structureSubsystem.getStructuralElementInstance(), domain));
		cmd.append(DLRCEFXStudyCommandHelper.createAddChildSEICommand(system.getStructuralElementInstance(),
				dataHandlingSubsystem.getStructuralElementInstance(), domain));
	...
	/**
	 * 
	 * @param disciplineList list of the current available disciplines
	 * @param disciplineName name of the discipline which should be added
	 * @return index of the discipline or -1 if the discipline with the given
	 *         disciplineName does not exist in the disciplineList
	 */
	private static int getIndexOfDisciplineAlreadyExists(EList<Discipline> disciplineList, String disciplineName) {
		return IntStream.range(0, disciplineList.size())
				.filter(i -> disciplineList.get(i).getName().equals(disciplineName)).findFirst().orElse(-1);
	}

	/**
	 * Handles the aggregation of a discipline
	 * 
	 * @param domain         current virsat domain
	 * @param disciplineName name of the discipline which should be aggregated
	 * @param compoundCmd    command to add to if a discipline has to be created
	 * @return a new discipline if the name is not already existing, if not it
	 *         creates a new one and appends a add command
	 */
	private static BeanDiscipline handleDisciplineCreation(VirSatTransactionalEditingDomain domain,
			String disciplineName, CompoundCommand cmd) {
		...
		BeanDiscipline discipline = index == -1 ? DLRCEFXStudyCommandHelper.createDiscipline(domain, disciplineName)
